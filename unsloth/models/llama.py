    ...
    def from_pretrained(
        ...
        **kwargs,
        ...
        try:
            ...
            raise RuntimeError('2Unsloth currently does not support multi GPU setups - but we are working on it!')
        pass

        if ((post_check - pre_check) >= 1).sum() > 1:
            raise RuntimeError('3Unsloth currently does not support multi GPU setups - but we are working on it!')
        ...
        if ((a - PRE_CHECK) >= 1).sum() > 1:
            raise RuntimeError('4Unsloth currently does not support multi GPU setups - but we are working on it!')
        for _ in range(3):
            gc.collect()

    ...
    def from_pretrained(
        ...
        **kwargs,
        ...
        try:
            ...
            raise RuntimeError('llama.py:1637 Unsloth currently does not support multi GPU setups - but we are working on it!')
        pass

        if ((post_check - pre_check) >= 1).sum() > 1:
            raise RuntimeError('llama.py:1641 Unsloth currently does not support multi GPU setups - but we are working on it!')
        ...
        if "," in index_for_cuda:
            raise RuntimeError("llama.py:1669 Unsloth currently does not support multi GPU setups - but we are working on it!")
        ...
        if ((a - PRE_CHECK) >= 1).sum() > 1:
            raise RuntimeError('llama.py:1682 Unsloth currently does not support multi GPU setups - but we are working on it!')
        for _ in range(3):
            gc.collect()
        ...
        if "n_total_devices >" not in inner_training_loop:
            raise RuntimeError('5Unsloth currently does not support multi GPU setups - but we are working on it!')
        pass
        inner_training_loop = inner_training_loop.replace(
